		...
		function(){
			if(nRow==null){
				...
				return false;
				 ...
				 wzj.confirm('温馨提示','是否通过该任务？',function(flag){
					var taskId=$("#result").children("tr").eq(nRow).children("td").eq(0).text();
					if(flag){
						$.post(
							"${ctx}/recognition/report/issueReport.action",
							"taskId="+taskId+"&result=true",
							function(data){//刷新页面
...
function view(){
	if(selectId==null){
		...
		//location.href = "${ctx}/application/toView.action?titleNo="+selectNo;
...
function show(){
	if(selectId==null){
		...
		location.href = "${ctx}/pbtSample/toSampleReports.action?titleNo="
				+selectNo+"&taskId="+taskId1+"&comName="+comName;
	}
}
                        ...
                        <table class="table table-bordered" style="width:460%" id="editable-sample">
                           <thead>
                              <tr>
                                 ...
                                 <th class="" style="display:none;">项目编号</th>
                                 <th class="">编号</th>
                                 <th class="">流水号</th>
                                 <th class="">分支机构</th>
                                 <th class="">企业名称</th>
