...
<script type="text/javascript">
	$(function(){
		$('#birthday').datepicker({
            ...
            minViewMode:1,
            viewMode: 1,
            viewDate:new Date(Date.parse("1970-01-01")),
            date:new Date(Date.parse("1970-01-01")),
		});
...
// 		);
    ...
    function userManage(){
    	...
    	param="dept="+"中心用户"+"&ramusCenter="+branch;
    	$.ajaxSetup({async : false});
		$.post(
	...
	function exportRecord(){
		window.open('${ctx}/sysUser/exportRecord.action?dept=中心用户');	
	}
...
//								$("#locked").val(data.locked);
							...
							$("#birthday").datepicker( 'setDate' , new Date(Date.parse(this.element.val())) )
					});
//				$("#username").val($("#name").val());
	...
	function updateUser(uid){
		...
		if(uid==""){
			...
			+"&password="+password+"&role="+roles+"&dept=中心用户&locked="+locked+"&ramusCenter=0000"
