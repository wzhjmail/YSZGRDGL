...
<style type="text/css">
	...
	var oTable = {
			...
			"bProcessing": true,
		    "bServerSide": true,	 
		    "sAjaxSource":"${ctx}/sysUser/findAll2.action?dept=分中心用户&ramusCenter=${activeUser.ramusCenter}",
		    "aLengthMenu": [
		                    [5, 15, 20, -1],
		                    [5, 15, 20, "All"]
		                ],
...
// 	    }
    ...
    function userManage(){
    	...
    	param="dept=分中心用户&ramusCenter="+branch;
    	$.ajaxSetup({async : false});
		$.post(
			"${ctx}/sysUser/findAll2.action",
			param,
			function(data){
				if(data!=null){
					for(var i=0;i<data.length;i++){
...
// 									var ramusCenterName=getDivName(data.ramusCenter);
// 									$("#ramusCenter").val(ramusCenterName);
...
//	 				$("#username").val($("#name").val());
	...
	function updateUser(uid){
		...
		if(uid==""){
			...
			+"&password="+password+"&role="+roles+"&dept=分中心用户&locked="+locked+"&ramusCenter="+ramusCenterId
			+"&gender="+gender+"&birthday="+birthday+"&phone="+phone+"&mobile="+mobile+"&email="+email+"&id="+$("#cid").val();
			$.post(//此处不能添加${pageContext.request.contextPath}
                 	"${ctx}/sysUser/insert.action",
                 	param,
                 	function(data){//返回值是添加用的ID
                 		wzj.confirm('温馨提醒','添加成功！',function(){
                 			if($("#branch").val()=="0000"){
                 				$("#name").val("");
                     			location.href="${ctx}/second.action";
             				}else{
             					$("#name").val("");
			...
			+"&password="+password+"&role="+roles+"&dept=分中心用户&locked="+locked+"&ramusCenter="+ramusCenterId
			+"&gender="+gender+"&birthday="+birthday+"&phone="+phone+"&mobile="+mobile+"&email="+email;
			$.post(
                     ...
                     <div class="adv-table editable-table ">
                        <div class="clearfix">
                           ...
                           <shiro:hasRole name="admin">
                              <button class="btn btn-success" data-toggle='modal' href='#Modal_show' onClick='toUpdateUser("")' id="Add">
         ...
         <!--  <div id="Modal_show" class="modal fade">
          <div class="modal-dialog">
			...
			<label class="col-sm-2 labels">用户名<b class="red">*</b>：</label>
                                 <div class="col-sm-4">
                                    <input type="text" id="usercode" name="usercode" value="" class="form-control validate[required]">
                                 </div>
			<label class="col-sm-2 labels">真实姓名<b class="red">*</b>：</label>
                                 <div class="col-sm-4">
                                    <input type="text" id="username" name="username" value="" class="form-control validate[required]">
                                 </div>
                                  </div>
             <div class="row" style="width:100%">
              <label class="col-sm-2 labels">密码<b class="red">*</b>：</label>
              <div class="col-sm-4">
                 <input type="text" id="password" name="password" value="" class="form-control validate[required]">
              </div>
               </div>
             <div class="row" style="width:100%">
              <label class="col-sm-2 labels">审核通过：</label>
              <div class="col-sm-4">
               ...
               第六行
			<div class="row" style="width:100%;display:none" id="toAdd">
