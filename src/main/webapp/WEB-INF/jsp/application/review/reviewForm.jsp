...
$(function(){
	...
	$.ajax({
		...
		success: function (data) {
			...
			str = '<form method="post" id="psjgForm'+i+'" action="" class="form-horizontal tasi-form"><div class="form-group"><label class="col-sm-4 control-label">'+items.required+'</label>'+
          		'<div class="col-sm-3 control-label"><br>'+items.method+'</div>'+
	 		'<div class="col-sm-3 "><input type="hidden" value="'+items.num+'" name="num"><br><textarea class="form-control validate[required]"  id="PSJL'+items.num+'" name="psjl"></textarea></div>'+
      			'<span class="col-sm-2" id="PSJG'+items.num+'"></span><br></div></form><hr>'
			$('#info').append(str)
			$("#psjgForm"+i).validationEngine({
...
// 				dispaly=items.num+"_"+resultnote[j];
// 				$("#info .form-group:last span:eq(0)").append('<input onclick="autoFill(this.id)" type="radio" name="psjg" class="validate[required] a" id="'+dispaly+'" value="'+result[j]+'">'+result[j]+'<br>')
// 			}
//给单选按钮添加id
			for(var j=0;j<result.length-1;j++){
				dispaly=items.num+"_"+result[j];
				$("#info .form-group:last span:eq(0)").append('<input onclick="autoFill(this.id)" type="radio" name="psjg" class="validate[required] a" id="'+dispaly+'" value="'+result[j]+'">'+result[j]+'<br>')
			}
			})
...
// 				$('#'+num1[i]+'_'+psjl[i]).attr('checked','checked')
// 			}
// 		}
			//回显评审结果跟结论
			if($("#num1").val()!=""&&$("#psjl").val()!=""&&$("#psjg").val()!=""){
...
<!--                         <select id="addzj"  onchange="addZj()"><option selected="true" disabled="true">全部</option></select> -->
                         <label class="col-sm-3 mtop">工作单位</label>
                         <label class="col-sm-3 mtop">职务/职称</label>
                         <label class="col-sm-2 mtop">是否签名</label>
                         <label class="col-sm-1 mtop">移除</label>
			  			</div>
...
<!-- 			  			</div> -->
                  ...
                  <div class="col-lg-12">
                     <section class="panel">
                        <header class="panel-heading">
                           ...
                           <spam class="label label-primary" onclick="frameworkIn()" href="">自动生成</spam>
                           <span class="tools pull-right" style="margin-top:-8px;">
                           <a href="javascript:;" class="fa fa-chevron-down"></a>
                           </span>
                        </header>
               ...
               <%-- <div class="row">
                  <div class="col-lg-12">
                     <section class="panel">
                        ...
                        <div class="panel-body">
                           ...
                           <div class="form-group">
                             <div class="col-sm-2 text-right"> 批准人签名：</div>
                             <div class="col-sm-3">
                                 <input type="text" id="pzman" name="pzman" class="form-control" value="${item.pzman }">
                             </div>
                             <div class="col-sm-1"></div>
                             <div class="col-sm-2 text-right"> 核准人签名：</div>
                             <div class="col-sm-3">
                                   <input type="text" id="hzman" name="hzman" class="form-control" value="${item.hzman }">
                             </div> 
                             <div class="col-sm-2 text-right" style="margin-top: 15px">编码中心盖章：</div>
                             <div class="col-sm-5"></div>
                             <div class="col-sm-1 text-right" style="margin-top: 15px">日期：</div>
                             <div class="col-sm-3" style="margin-top: 15px">
                        		<input class="input-medium default-date-picker form-control" size="16" id="pzdate" name="pzdate" value="${item.pzdate}">
                      		 </div>
                           </div>
			  			</div>
                     </section>
                 ...
                 <div class="row">
	               ...
	               <div class="panel-body text-center">
	               			...
	               			<input type="text" style='display:none;' id="titleNum" value="${item.titleno}">
		 ...
		 <!-- END MAIN CONTENT -->
               ...
               <div id="Modal_show" class="modal" style="margin-top: 10%;margin-left: -18%;overflow-y: hidden;">
                     ...
                     <section class="panel">
                        <div class="panel-body">
                           ...
                           <div class="form-group text-center">
                              ...
                              <div class="form-group">
                                 <div class="col-sm-6 text-right labels">评审表：</div>
                                 <div class="col-sm-2">
                             ...
                             <form id="forms" method="post" target="nm_iframe" action="" enctype="multipart/form-data">
                              <div class="form-group">
                                 <div class="col-sm-6 text-right labels">评审表（盖章版）<b class="red">*</b>：</div>
                                 <div class="col-sm-4">
<!--                                    <button class="btn btn-info" onclick="upfunc()">上传</button> -->
                            ...
                            <!--   <div class="form-group">	
                              <div class="col-lg-5 col-sm-5"></div>
			               		...
			               		<input type="button" id="nextbtn2" onclick="nextStep(2)" class="btn-success btn-block btn " value="下一步"></div>
			               		<div class="col-lg-5 col-sm-5"></div>	
			               	</div> -->			 
                           </span>
                        </div>
        ...
        function submitForm(){
        	if($('#revInfo').validationEngine('validate')){
       	    ...
       	    if($('#revForm').validationEngine('validate')){
	        	...
	        	$.post(
						"${ctx}/application/review/insertReview.action",
						...
						function(data){
							...
							for(var i=0;i<nums.length;i++){
								dat+="reviewFormParts["+i+"].num="+$(nums[i]).val()+"&reviewFormParts["+i+"].psjl="+$("#PSJL"+$(nums[i]).val()).val()+"&reviewFormParts["+i+"].psjg="+$(psjg[i]).val()+"&reviewFormParts["+i+"].syndicid="+data+"&";
							}
							dat=dat.substring(0,dat.length-1);
							$.post(
									"${ctx}/application/review/insertReviewPart.action",
									dat,
									function(data){
										$("body").mLoading("hide")
...
//			);
	    ...
	    function nextStep(num){
			...
			var taskId=$("#taskId").val();
			$.post(
				"${ctx}/application/review/savePDF.action",
				"taskId="+taskId,
				function(){
...
// 			$("#forms").submit();
			...
			if(file=="未上传文件"){
				...
				$("body").mLoading("show");
				$('#forms').attr("action","${ctx}/application/review/applyReview.action");
				$("#forms").submit();
...
// 			if($("#sq").val()!=""&&$("#titleNum").val()!=""&&$("#appId").val()!=""){
...
// 			}
		...
		function jump(){
			location.href="${ctx}/application/review/toPersonalTask.action";
		}  
