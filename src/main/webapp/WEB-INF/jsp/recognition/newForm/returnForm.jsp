...
function getTask(){
	...
	if(id!=null){//如果id存在
		...
		$.post(
				...
				param,
				function(data){
					getTestingEquip(data.companyId);//根据companyId获取条码检测设备。
                    getPrintEquipment(data.companyId);//根据companyId获取主要印刷设备
...
function getPrintEquipment(companyId) {//查看主要印刷设备
    $.post(
...
function toAddPrinting() {//添加新的主要印刷设备
    $("#iPrintName").val("");
    $("#iPrintModel").val("");
    $("#iPrintPlace").val("");
    $("#iPrintNumber").val("");
...
function addPrinting() {
	...
	if($('#addPrint').validationEngine('validate')){
	    ...
	    var param="companyId="+companyId+"&companyName="+companyName
	        +"&printName="+printName+"&printModel="+printModel
	        +"&printPlace="+printPlace+"&printNumber="+printNumber
	        ...
	        function(data){
	            if(data > 0){
	                ...
	                $("#printing").append("<tr><td style='display:none'>"+data+"</td><td>"+ printName+"</td>"+
	                    "<td>"+ printModel+"</td><td>"+printPlace+"</td><td>"+printNumber+"</td><td>"+printNotes+
	                    "</td><td><button onclick='toUpdatePrinting(this)' class='btn btn-info'>修改</button></td>"+
	                    "<td><button onclick='deletePrinting(this)' class='btn btn-danger'>删除</button></td></tr>");
	            }
	        }
...
function toUpdatePrinting(that){//修改主要印刷设备
    ...
    $.post(
        "${ctx}/application/printEquipment/getById.action",
        "id="+printingId,
        function(data){
            $("#pId").val(printingId);
...
function updatePrinting(){
	...
	if($('#updatePrint').validationEngine('validate')){
	    ...
	    var comName=$("#enterprisename").val();
	    var comId=$("#companyId").val();
	    console.log("id="+pId+"&printName="+uPrintName+"&printModel="+uPrintModel+"&printPlace="+uPrintPlace+"&printNumber="+uPrintNumber+"&printNotes="+uPrintNotes+"&companyName="+comName+"&companyId="+comId);
	    $.post(
	        "${ctx}/application/printEquipment/updateById.action",
	        "id="+pId+"&printName="+uPrintName+"&printModel="+uPrintModel+"&printPlace="+uPrintPlace+"&printNumber="+uPrintNumber+"&printNotes="+uPrintNotes+"&companyName="+comName+"&companyId="+comId,
	        function(data){
...
function deletePrinting(that) {//删除主要印刷设备
...
function addTesting(){
	...
	if($('#toAddTesting').validationEngine('validate')){
		...
		var param="name="+name+"&testingModel="+model+"&count="+count+"&time="+time+"&remark="
			+remark+"&companyId="+companyId+"&companyName="+companyName;
		$.post(
				...
				function(data){
					if(data>0){
						...
						$("#testing").append("<tr><td style='display:none'>"+data+"</td><td>"+
								name+"</td><td>"+model+"</td><td>"+count+"</td><td>"+time+"</td><td>"+remark+
								"</td><td><button onclick='toUpdateTesting(this)' class='btn btn-info'>修改</button></td>"+
								"<td><button onclick='deleteTesting(this)' class='btn btn-danger'>删除</button></td></tr>");
					}
				}
...
function updateTesting(){
	...
	if($('#updateTesting').validationEngine('validate')){
		...
		var	conpanyName=$("#enterprisename").val();
		var companyId=$("#companyId").val();
		$.post(
			"${ctx}/application/testingEquip/updateById.action",
			"id="+tid1+"&name="+tname1+"&testingModel="+tmodel1+"&count="+tcount1+"&time="+time
				+"&remark="+tremark1+"&companyName="+conpanyName+"&companyId="+companyId,
			function(data){
...
function check(show){
	...
	var $img;
	var titleno;
	if(taskId!=null && taskId!='null'){//退回任务的保存
		titleno=$('#titleno').val()
	}else{
		titleno=$('#titleno').val().replace('FR', 'XB');
	}
                           ...
                           <span class="form-horizontal tasi-form">
                              <div class="form-group">
                                 ...
                                 <div class="col-sm-6">
                                    ...
                                    <input type="hidden" id="name" name="name" value="${item.enterprisename}">
                              ...
                              <div class="form-group">
                                 ...
                                 <div class="col-sm-4">
	                          	...
	                          	<label class="col-sm-1 control-label labels">邮政编码<b class="red">*</b></label>
	                            <div class="col-sm-2">
							  ...
							  <div class="form-group">
								 ...
								 <label class="col-sm-2 control-label labels">企业性质<b class="red">*</b></label>
                                 <div class="col-sm-2">
			  ...
			  <div class="row">
                           ...
                           <span class="form-horizontal tasi-form">
                              <div class="form-group">
								 ...
								 <label class="col-sm-2 checkbox-inline"></label>
                                    ...
                                    <input type="checkbox" class="validate[funcCall[minCheckbox]]" data-flag="grp1" name="flat" id="flat" value="true" <c:if test="${item.gravure==true}">checked="checked"</c:if>>平板胶印
                                    </label>
                                    <label class="col-sm-2 checkbox-inline">
						   ...
						   <div class="form-group">
                             <div class="col-sm-12">
	                             ...
	                             <div class="col-lg-3">
	                             	<div class="col-sm-4">
	                             		...
	                             		<p id="em1" style="float: left;padding: 7px;margin: 0">${yyzz}</p>
	                             	</div>
                             ...
                             <div class="col-lg-3">
                             	<div class="col-sm-4">
                             		...
                             		<p id="em2" style="float: left;padding: 7px;margin: 0">${yszg}</p>
                             	</div>
                           ...
                           <div class="col-sm-12">
                             	...
                             	<div class="col-lg-3">
                             		<div class="col-sm-4">
	                             		...
	                             		<p id="em3" style="float: left;padding: 7px;margin: 0">${zlsc}</p>
	                             	</div>	
			   ...
			   <div class="row" id="btn1">
	               ...
	               <div class="panel-body text-center">
	               		<div class="btn-group">
	               			<input type="button" onclick="saveForm()" class="btn-info btn" value="保存">
	               		</div>
	               		<div class="btn-group">
	               			<input type="button" id="nextbtn1" onclick="nextStep(1)" class="btn-success btn disabled" value="下一步">
	               		</div>
	               </div>
	               </section>
	               </div>
	    ...
	    <!-- END SECTION -->
	              ...
	              <div class="modal-body" style="height: 200px">
	               ...
	               <div class="row">
		              <div class="form-group">
		              	...
		              	<div class="col-sm-3"><input name="name" id="tname1" type="text" class="form-control validate[custom[username]]"></div>
		              	<div class="col-sm-2 labels"><b class="red">*</b> 设备型号：</div>
		              	<div class="col-sm-3"><input name="testingModel" id="tmodel1" type="text" class="form-control"></div>
		              </div>
		              </div>
		              <br>
		               <div class="row">
		              <div class="form-group">
		              	...
		              	<div class="col-sm-3"><input name="time" id="ttime1" type="text" class="form-control validate[custom[date]]"></div>
		              	<div class="col-sm-2 labels"><b class="red">*</b> 数量：</div>
		              	<div class="col-sm-3"><input name="count" id="tcount1" type="text" class="form-control validate[custom[plus]]"></div>
		              </div>
		              </div><br>
		              <div class="row">
		              <div class="form-group">
                      ...
                      <div class="modal-content">
                          ...
                          <div class="modal-body" style="height: 200px">
                              ...
                              <div class="row">
                                  <div class="form-group">
                                      <div class="col-sm-3 labels"><b class="red">*</b> 设备名称：</div>
                                      <div class="col-sm-3"><input name="PrintName" id="iPrintName" type="text" class="form-control validate[custom[username]]"></div>
                                      <div class="col-sm-2 labels"><b class="red">*</b> 设备型号：</div>
                                      <div class="col-sm-3"><input name="printModel" id="iPrintModel" type="text" class="form-control"></div>
                                  </div>
                              </div>
                              <br>
                              <div class="row">
                                  <div class="form-group">
                                      ...
                                      <div class="col-sm-3"><input name="printPlace" id="iPrintPlace" type="text" class="form-control validate[custom[username]]"></div>
                                      <div class="col-sm-2 labels"><b class="red">*</b> 设备数量：</div>
                                      <div class="col-sm-3"><input name="printNumber" id="iPrintNumber" type="text" class="form-control validate[custom[plus]]"></div>
                                  </div>
                              </div>
                              <br>
                              <div class="row">
              ...
              <div id="Modal_show6" class="modal fade" style="margin-top:12%">
                  <div class="modal-dialog">
                      <div class="modal-content">
                          <div class="modal-header">
                              <button type="button" class="close" data-dismiss="modal" aria-hidden="true" id="updateP">
                                  &times;
                              </button>
                              <h4 class="modal-title">主要印刷设备</h4>
                          </div>
                          <div class="modal-body" style="height: 200px">
                              ...
                              <div class="row">
                                  <div class="form-group">
                                      ...
                                      <div class="col-sm-3"><input name="PrintName" id="uPrintName" type="text" class="form-control validate[custom[username]]"></div>
                                      <div class="col-sm-2 labels"><b class="red">*</b> 印刷设备型号：</div>
                                      <div class="col-sm-3"><input name="printModel" id="uPrintModel" type="text" class="form-control"></div>
                                  </div>
                              </div>
                              <br>
                              <div class="row">
                                  <div class="form-group">
                                      ...
                                      <div class="col-sm-3"><input name="printPlace" id="uPrintPlace" type="text" class="form-control validate[custom[username]]"></div>
                                      <div class="col-sm-2 labels"><b class="red">*</b> 印刷设备数量：</div>
                                      <div class="col-sm-3"><input name="printNumber" id="uPrintNumber" type="text" class="form-control validate[custom[plus]]"></div>
                                  </div>
                              </div>
                              <br>
                              <div class="row">
	            ...
	            <div class="modal-content">
	              ...
	              <form action="" method="post" id="toAddTesting"  enctype="multipart/form-data">
	               <div class="row">
		              <div class="form-group">
		              	<div class="col-sm-3 labels"><b class="red">*</b> 设备名称：</div>
		              	<div class="col-sm-3"><input name="name" id="tname" type="text" class="form-control validate[custom[username]]"></div>
		              	<div class="col-sm-2 labels"><b class="red">*</b> 设备型号：</div>
		              	<div class="col-sm-3"><input name="testingModel" id="tmodel" type="text" class="form-control"></div>
		              </div>
		              </div>
		              <br>
		               <div class="row">
		              <div class="form-group">
		              	<div class="col-sm-3 labels"><b class="red">*</b> 最近一次校准日期：</div>
		              	<div class="col-sm-3"><input name="time" id="ttime" type="text" class="form-control validate[custom[date]]"></div>
		              	<div class="col-sm-2 labels"><b class="red">*</b> 数量：</div>
		              	<div class="col-sm-3"><input name="count" id="tcount" type="text" class="form-control validate[custom[plus]]"></div>
		              </div>
		              </div><br>
		              <div class="row">
		              <div class="form-group">
                        ...
                        <div class="panel-body">
                           <span class="form-horizontal tasi-form">
                             ...
                             <form id="forms" method="post" target="nm_iframe" action="" enctype="multipart/form-data">
                              <div class="form-group">
                                 <div class="col-sm-6 text-right lebels">申请表（盖章版）<b class="red">*</b>：</div>
                                 <div class="col-sm-6">
	...
	<script src="${ctx}/js/common-scripts.js" ></script><!-- BASIC COMMON JS  -->
     ...
     <script>
        ...
        jQuery(document).ready(function() {
    	    ...
    	    "${yyzz}" == ""?($("#em1").text("未上传文件")):($("#em1").text("${yyzz}"))
    	    "${yszg}" == ""?($("#em2").text("未上传文件")):($("#em2").text("${yszg}"))
    	    "${zlsc}" == ""?($("#em3").text("未上传文件")):($("#em3").text("${zlsc}"))
        });
					...
					if(data>1){
						wzj.alert('温馨提醒', '该企业已进行申请！')
			 			...
			 			if($("#name").val()==enterpriseName){
			 		        	...
			 		        	if(taskId!=null && taskId!='null'){//退回任务的保存
			 		        		$("#appForm").attr("action","${ctx}/recognition/saveReturn.action?oldName="+$("#name").val());
			 		        	}else{//新任务的保存
			 		        		$("#appForm").attr("action","${ctx}/recognition/save.action?oldName="+$("#name").val());
			 		        	}
			 	        	$("#appForm").submit();
			 	        	wzj.alert('温馨提醒', '保存成功，请进行下一步！')
			 				$('#nextbtn1').removeClass('disabled')
			 			}else{
	// 		 				if(cId==$("#companyId").val()){
	// 			 		        	var taskId=$("#taskId").val();
	// 			 		        	if(taskId!=null && taskId!='null'){//退回任务的保存
	// 			 		        		$("#appForm").attr("action","${ctx}/recognition/saveReturn.action?oldName="+$("#name").val());
	// 			 		        	}else{//新任务的保存
...
// 	        	var taskId=$("#taskId").val();
// 	        	if(taskId==0){//新任务的提交
// 	        		$("#appForm").attr("action","${ctx}/recognition/insert.action");
// 	        	}else{//退回任务的提交
// 	        		$("#appForm").attr("action","${ctx}/recognition/insertReturn.action");
// 	        	}
// 	        	$("#appForm").submit();
//         	}
//         }
	    ...
	    function nextStep(num){
	    	...
	    	$.post(
	    		"${ctx}/recognition/getParam.action",
	    		"name="+ename,
	    		function(data){
		    		console.log(data)
	    			$("#titleNo").val(data.titleno);
			 		$("#appId").val(data.id);
			 		$("#em").html(data.appFileName);
	    		}
	    	)
			$('#appForm').hide()
			$('#load1').hide()
...
// 				wzj.alert('温馨提醒', '下载成功！')
	    ...
	    function upfunc(){
	    	...
	    	if(file=="未上传文件"){
				...
				$('#forms').ajaxSubmit({  
	            type:'post',  
	            url: '${ctx}/recognition/startApply.action',  
