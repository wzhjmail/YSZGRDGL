!function($) {
	...
	Datepicker.prototype = {
		...
		show : function(e) {
			...
			if(this.picker.find('.datepicker-days .table-condensed tbody tr .active').length!=0){
				this.set()
			}
		...
		fill : function() {
			...
			this.picker.find('.datepicker-days th:eq(1)').text(//修改
					year + '年 ' + DPGlobal.dates.months[month]);
				...
				case 'td':
					if (target.is('.day')) {
						if(target.is('.active')){
							...
							console.log(this.element.find('input').context.value)
							this.element.find('input').context.value="";
							//this.hides();
							this.element.blur()
						}else{
